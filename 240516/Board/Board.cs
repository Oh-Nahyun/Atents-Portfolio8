...
public class Board : MonoBehaviour
{
    ...
    /// </summary>
    public Dictionary<ShipType, Action> onShipAttacked; ///// 딕셔너리 : 키값이 있는 트리
    ...
    /// </summary>
    /// <param name="ship">배치할 함선 (위치, 방향, 크기 등의 정보 사용)</param>
    /// <param name="world">배치될 위치 (함선의 머리 위치, 월드 좌표)</param>
    ...
    /// <param name="grid">확인할 배 위치 (함선의 머리 위치, 그리드 좌표)</param>
    ...
    public bool IsShipDeploymentAvailable(Ship ship, Vector2Int grid, out Vector2Int[] resultPositions)
    {
        ...
        switch (ship.Direction)             // 방향 벡터 값 결정
        {
        ...
        bool result = true;                         // 모든 위치가 배치 가능하면 true
        foreach (var pos in resultPositions)        // 확인할 위치들을 하나씩 확인하기
        {
            if (!IsInBoard(pos) || IsShipDeployedPosition(pos)) // 하나라도 보드 밖이거나 배가 배치되어있으면
            {
                result = false;                     // 배치 불가능
                break;
            }
    ...
    /// </summary>
    /// <param name="ship">확인할 배</param>
    /// <param name="world">확인할 배 위치 (함선의 머리 위치, 월드 좌표)</param>
    /// <returns>true면 배치 가능, false면 배치 불가능</returns>
    ...
    /// 보드의 특정 위치에 있는 배의 종류를 확인하는 함수
    /// </summary>
    /// <param name="grid">확인할 위치(월드 좌표)</param>
    /// <returns>배가 없거나 보드 밖이면 ShipType.None, 배가 있으면 그 배의 종류</returns>
    ...
    /// </summary>
    /// <param name="grid">공격 받은 위치 (그리드 좌표), 이 위치는 반드시 보드 안쪽이고 공격 가능한 위치이다.</param>
    ...
    public bool OnAttacked(Vector2Int grid)
    {
        ...
        bombSetter.SetBombMark(GridToWorld(grid), result); // 붐마크로 표시하기
    ...
    public bool IsAttackFailPosition(Vector2Int grid)
    {
        ...
        // grid가 보드 안쪽이고, 공격 당한 위치이고, 배가 없다.
        return index != null && isAttacked[index.Value] && shipInfo[index.Value] == ShipType.None;
    }
    ...
    /// <param name="x">그리드의 x좌표</param>
    /// <param name="y">그리드의 y좌표</param>
    /// <returns>계산된 월드 좌표 (그리드의 중심점)</returns>
