...
public class Ship : MonoBehaviour
{
    ...
    public ShipDirection Direction
    {
        ...
        {
            direction = value;
            modelRoot.rotation = Quaternion.Euler(0, (int)direction * 90.0f, 0); // 방향이 변경되면 방향에 맞게 회전 ///// modelRoot 회전
        }
    }
    ...
    public void SetMaterialType(bool isNormal = true)
    {
        ...
        {
            shipRenderer.material = ShipManager.Instance.DeployModeShipMaterial;
        }
    }
    ...
    public void Rotate(bool isCW = true)
    {
        ...
        {
            // 음수 % 연산이 일어나는 것을 방지
            // % 연산을 할 때는 나누는 숫자를 몇 번을 더해도 결과에 영향을 주지 않는다.
    ...
    public void OnHit()
    {
        ...
        if (IsAlive)
        {
            onHit?.Invoke(this); // 명중을 알리는 것은 공격을 당하고 죽지 않았을 때만 처리
        }
    }
