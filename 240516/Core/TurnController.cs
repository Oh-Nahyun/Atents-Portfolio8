...
public class TurnController : MonoBehaviour
{
    ...
    /// <summary>
    /// 이번 턴이 타임아웃될 때까지 남아있는 시간
    /// </summary>
    float turnRemainTime = 0.0f;

    /// <summary>
    /// 타임아웃이 활성화되어 있는지 표시
    /// (false면 타임아웃이 일어나지 않는다.)
    /// </summary>
    ...
    /// <summary>
    /// 턴이 진행될지 여부
    /// (true면 턴이 진행되고, false면 턴이 진행되지 않는다.)
    /// </summary>
    ...
    {
        turnNumber = 0;                     // OnTurnStart에서 turnNumber를 증가시키기 때문에 0에서 시작

        if (!timeOutEnable)                 // 타임아웃을 껐으면
        {
            turnDuration = float.MaxValue;  // turnDuration을 매우 길게 잡기
        }
        turnRemainTime = TurnDuration;      // 턴 남은 시간을 turnDutation으로 설정
        ...
        ////////////////////////////////////////////////////////////////////////
        /// 이 부분은 안 좋은 코드 (이유 : 쌍방으로 불러와서 사용하고 있기 때문)
    ...
    void OnTurnStart()
    {
        if (isTurnEnable)                       // 턴 매니저가 작동 중이면
        {
            ...
            turnRemainTime = TurnDuration;      // 타임아웃용 시간 리셋
