...
public class Test_07_BoardAttack : Test_06_AutoShipDeployment
{
    protected override void Start()
    {
        ...
        board.onShipAttacked[ShipType.Carrier] += GetShip(ShipType.Carrier).OnHit;
        board.onShipAttacked[ShipType.BattleShip] += GetShip(ShipType.BattleShip).OnHit;
        board.onShipAttacked[ShipType.Destroyer] += GetShip(ShipType.Destroyer).OnHit;
        board.onShipAttacked[ShipType.Submarine] += GetShip(ShipType.Submarine).OnHit;
        board.onShipAttacked[ShipType.PatrolBoat] += GetShip(ShipType.PatrolBoat).OnHit;
