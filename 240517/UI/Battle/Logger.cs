...
public class Logger : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            ship.onHit += (target) => Log_AttackSuccess(false, target); // 유저의 함선이 명중되었으니 적의 공격이 성공한 것을 의미
            ship.onSink += (target) => Log_ShipSinking(false, target);
        }
        foreach (Ship ship in enemy.Ships)
        {
            ship.onHit += (target) => Log_AttackSuccess(true, target); // 적의 함선이 명중되었으니 유저의 공격이 성공한 것을 의미
            ship.onSink += (target) => Log_ShipSinking(true, target);
        }
    ...
    void Log_AttackSuccess(bool isUser, Ship ship)
    {
        ...
        Log($"<b><#{attackerColor}>[{attackerName}]</color></b>의 공격\t: <b><#{hitterColor}>[{hitterName}]</color></b>의 <b><#{shipTextColor}>[{ship.ShipName}]</color></b>에 포탄이 명중했습니다.");
    }
...
/// 2. 턴 시작 시에 몇번째 턴이 시작되었는지 출력하기 (턴 번호는 다른 색으로 출력하기)
/// 3. 공격 시에 공격 성공 / 실패 출력하기 (공격 실패 델리게이트 필요)
...
// 내가 작성해본 코드
//TextMeshProUGUI loggerText;
//PlayerBase playerBase;
//TurnPrinter turnPrinter;

//private void Awake()
//{
//    loggerText = GetComponent<TextMeshProUGUI>();
//    playerBase = FindAnyObjectByType<PlayerBase>();
//    turnPrinter = FindAnyObjectByType<TurnPrinter>();
//}

//private void Start()
//{
//    loggerText.text = $"{turnPrinter.turn} 턴 시작";
//    playerBase.onAttackFail += () =>
//    {
//        loggerText.text = $"{playerBase.name}의 공격 : {}의 {ship}에 포탄이 명중했습니다.";
//        loggerText.text = $"{}의 공격 : {}의 포탄이 빗나갔습니다.";
//        loggerText.text = $"";
//    };
//}
