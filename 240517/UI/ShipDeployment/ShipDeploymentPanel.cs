...
public class ShipDeploymentPanel : MonoBehaviour
{
    ...
    private void Awake()
    {
        toggles = GetComponentsInChildren<DeploymentToggle>(); // 모든 자식 토글 버튼 찾기
        foreach (DeploymentToggle toggle in toggles)
        {
            toggle.onSelect += UnSelectOthers; // 한 토글 버튼이 눌려지면 다른 모든 토글 버튼을 NotSelect 상태로 만들기
        }
    }

    /// <summary>
    /// self를 제외한 현재 토글 버튼을 NotSelect로 만드는 함수
    /// </summary>
