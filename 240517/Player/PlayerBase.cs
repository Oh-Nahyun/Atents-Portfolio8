...
public class PlayerBase : MonoBehaviour
{
    ...
    /// <summary>
    /// 이 플레이어의 공격이 실패했음을 알리는 델리게이트 (bool : true면 User, false면 Enemy)
    /// </summary>
    ...
    protected void Initialize()
    {
        ...
        remainShipCount = count;                                    // 함섬 개수 초기화
        ...
        turnController.onTurnStart += OnPlayerTurnStart;   // 턴 시작 함수 연결
        turnController.onTurnEnd += OnPlayerTurnEnd;       // 턴 종료 함수 연결
    ...
    public void Attack(Vector2Int attackGrid)
    {
        ...
        if (!isActionDone && opponentBoard.IsInBoard(attackGrid) && opponentBoard.IsAttackable(attackGrid))
        {
            ...
            {
                // 성공 -> 실패 -> 성공 순서였을 때 두 번째 성공에서 주변 모두를 추가하는 문제 수정용
    ...
    private void AddCriticalFromTwoPoint(Vector2Int now, Vector2Int last)
    {
        if (IsSuccessLine(last, now, true))
        {
            ...
            foreach (var index in criticalAttackIndices)
            {
                ...
                if (grid.y != now.y)                                        // y가 다르면 한 줄에 있는 것이 아니다.
                {
            ...
            // 양끝(수평)의 위치를 Critical에 추가
            grid = now;
            for (grid.x = now.x + 1; grid.x < Board.BoardSize; grid.x++)    // now의 오른쪽 확인 후 추가
            {
            ...
            for (grid.x = now.x - 1; grid.x > -1; grid.x--)                 // now의 왼쪽 확인 후 추가
            {
        ...
        else if (IsSuccessLine(last, now, false))
        {
            ...
            {
                grid = Board.IndexToGrid(index);
                if (grid.x != now.x)                                        // x가 다르면 한 줄에 있는 것이 아니다.
                {
                    deleteTarget.Add(index);
                }
            }
            ...
            grid = now;
            for (grid.y = now.y + 1; grid.y < Board.BoardSize; grid.y++)    // now의 아래쪽 확인 후 추가
            {
                if (!Board.IsInBoard(grid))
                    break;
                if (opponent.board.IsAttackFailPosition(grid))
                ...
                if (opponent.Board.IsAttackable(grid))
                {
                    AddCritical((uint)Board.GridToIndex(grid).Value);       // now의 위쪽 확인 후 추가
                    break;
                }
            }
        ...
        {
            // 같은 줄이 아니다. ( = 다른 배이다.) >> 이웃들을 추가한다.
            AddCriticalFromNeighbors(now);
        }
    }

    /// <summary>
    /// 한 줄로 공격이 성공했는지(start에서 end까지 모두 공격 성공이었는지)를 체크하는 함수
    /// </summary>
    ...
    private void AddCritical(uint index)
    {
        if (!criticalAttackIndices.Contains(index)) // 없을 때만 추가
        {
            ...
            if (gameManager.IsTestMode)
            {
                ...
                obj.name = $"Critical_({grid.x}, {grid.y})";                            // 이름 알아보기 쉽게 변경
    ...
    {
        if (criticalAttackIndices.Contains(index))  // 공격 후보 지역이 있으면
        {
            criticalAttackIndices.Remove(index);    // 공격 후보 지역에서 제거
        }
        ...
        if (criticalMarks.ContainsKey(index))       // 키가 있는지 확인
        {
            ...
            criticalMarks.Remove(index);            // 해당 키값 제거
        }
    }
    ...
    private void RemoveAllCriticalPositions()
    {
        ...
        criticalAttackIndices.Clear();              // 공격 후보 지역 리스트 초기화
        lastSuccessAttackPosition = NOT_SUCCESSS;
    }
    ...
    {
        if (!isActionDone) // 턴이 끝났는데도 아무 행동을 안했으면 자동 공격
        {
            AutoAttack();
        }
    }
